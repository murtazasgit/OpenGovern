...
@pytest.fixture()
def daocontract_client(
    algorand_client: AlgorandClient, deployer: SigningAccount
) -> DaocontractClient:
    factory = algorand_client.client.get_typed_app_factory(
        DaocontractFactory, default_sender=deployer.address
    )
...
def test_says_hello(daocontract_client: DaocontractClient) -> None:
    result = daocontract_client.send.hello(args=("World",))
    assert result.abi_return == "Hello, World"


def test_simulate_says_hello_with_correct_budget_consumed(
    daocontract_client: DaocontractClient,
) -> None:
    result = (
        daocontract_client.new_group()
        .hello(args=("World",))
        .hello(args=("Jane",))
        .simulate()
    )
